...
- `PolarsParquetIOManager` reads now consult the IO manager's own `storage_options` first and remap fsspec-style keys (`key`, `secret`, `token`, `client_kwargs.region_name`, `client_kwargs.endpoint_url`) into the `object_store`-style keys Polars expects (`aws_access_key_id`, `aws_secret_access_key`, `aws_session_token`, `aws_region`, `aws_endpoint_url`). Previously the read path passed fsspec keys straight to `pl.scan_parquet`, which `object_store` ignored — manifesting as `Generic S3 error: Missing bucket name` when downstream assets tried to load an upstream parquet from S3. Gated behind `polars >= 1.17`; older versions keep the legacy fsspec passthrough. Closes [#257](https://github.com/dagster-io/community-integrations/issues/257).
